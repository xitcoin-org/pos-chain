...
`,
	...
	// Keep the inter-block cache disabled by default. The Cosmos SDK cache
	// implementation can race with concurrent queries during block execution,
	// which can make otherwise identical validators compute divergent state.
	cmd.Flags().Bool(server.FlagInterBlockCache, false, "Enable inter-block caching")
