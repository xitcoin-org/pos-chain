...
```bash
go get github.com/xitcoin-org/pos-chain@v0.7.0
go mod tidy
```
...
Internal Cosmos EVM relocations — the top-level `github.com/xitcoin-org/pos-chain/config` package was deleted and its symbols redistributed:
...
The example-app helpers (`MustGetDefaultNodeHome`, `InitAppConfig`, `BlockedAddresses`, `GetMaccPerms`) moved from `github.com/xitcoin-org/pos-chain/config` to `github.com/xitcoin-org/pos-chain/evmd/config`. Mempool / chain-id helpers moved to `github.com/xitcoin-org/pos-chain/server` and `github.com/xitcoin-org/pos-chain/utils`. Update every import in `evmd/cmd/evmd/cmd/root.go`, `evmd/cmd/evmd/main.go`, and `evmd/cmd/evmd/cmd/testnet.go` accordingly.
...
Imports to add: `"github.com/cosmos/cosmos-sdk/baseapp/txnrunner"`, `vmrunner "github.com/xitcoin-org/pos-chain/x/vm/runner"`.
...
`server.ResolveMempoolConfig`, `server.GetCosmosPoolMaxTx`, and `server.GetMempoolCheckTxTimeout` are all in `github.com/xitcoin-org/pos-chain/server`; the `evmconfig.GetLegacyPoolConfig` / `GetBlockGasLimit` / `GetMinTip` helpers from v0.6 collapsed into `ResolveMempoolConfig`. `app.TxDecode` and the `SetInsertTxHandler` / `SetReapTxsHandler` setters are new in cosmos-sdk v0.54 baseapp.
