...
- Bump `github.com/xitcoin-org/pos-chain` to v0.5.0 and run:
...
## 2) Fix `"github.com/xitcoin-org/pos-chain/types" imports`

`v0.5.0` removes `github.com/xitcoin-org/pos-chain/types` and moves files to their folders, respective to function.
For a complete list of changes, refer to [this PR](https://github.com/xitcoin-org/pos-chain/pull/639). The
...
**Removed import:**
```diff
- cosmosevmtypes "github.com/xitcoin-org/pos-chain/types"
```
...
- **`AttoPowerReduction`** → moved to `"github.com/xitcoin-org/pos-chain/utils"`
  ```diff
  - sdk.DefaultPowerReduction = cosmosevmtypes.AttoPowerReduction
  + sdk.DefaultPowerReduction = utils.AttoPowerReduction
  ```

- **`HasDynamicFeeExtensionOption`** → moved to `"github.com/xitcoin-org/pos-chain/ante/types"`
  ```diff
  - ExtensionOptionChecker: cosmosevmtypes.HasDynamicFeeExtensionOption,
  + ExtensionOptionChecker: antetypes.HasDynamicFeeExtensionOption,
  ```

- **Address Codec functions** → new package `"github.com/xitcoin-org/pos-chain/encoding/address"`
...
- **`Bip44CoinType`, `BIP44HDPath`** → moved to `"github.com/xitcoin-org/pos-chain/crypto/hd"`

- **`GenesisState`** → removed as a duplicate object can be found in the `evmd` folder and a testing version is in `"github.com/xitcoin-org/pos-chain/testutil"`
...
```
- **After**: Using helper function (optional). See https://github.com/xitcoin-org/pos-chain/blob/42e1141c9b00432020e69945e565f60f80ab501d/evmd/mempool.go for the reference implementation
```go
...
PR [#496](https://github.com/xitcoin-org/pos-chain/pull/496) replaced pre-built pools with configs in `EVMMempoolConfig`:
...
PR [#698](https://github.com/xitcoin-org/pos-chain/pull/698) adds new configuration options for the EVM mempool that can be set via `app.toml` or CLI flags. These options allow fine-tuning of the EVM legacy pool behavior.
...
The --mempool.max-txs SDK flag is now actually ingested into the Cosmos mempool (`ExtMempool`) config inside `ExperimentalEVMMempool` (this was previously always a zeroed `int` value and could not be overridden). This allows limiting the maximum number of transactions in the Cosmos mempool. Set to -1 (disabled, no-op), 0 (unbounded), or positive integer. The [default value here](https://github.com/xitcoin-org/pos-chain/blob/f6ce772cd8dabfc5804aeb94ee21c63cc3d112fa/server/start.go#L189-L192) is `0`, which differs from the default SDK value of `-1`, since we want the mempool enabled by default.
...
`github.com/xitcoin-org/pos-chain/config` and may be removed to your repo.

### Denom Configs

[#661](https://github.com/xitcoin-org/pos-chain/pull/661) removes the instantiation of chain configs via app.go
...
Please refer to the [upgrade example](https://github.com/xitcoin-org/pos-chain/blob/main/evmd/upgrades.go) for the complete implementation.
