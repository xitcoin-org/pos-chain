...
```diff
- github.com/xitcoin-org/pos-chain v0.3.1
+ github.com/xitcoin-org/pos-chain v0.4.0
```
...
Update your app’s `newApp` should return an **`evmserver.Application // from evmserver "github.com/xitcoin-org/pos-chain/server"`** rather than `servertypes.Application`, and CLI commands that still expect an SDK app creator require a wrapper.
...
+) evmserver.Application // from evmserver "github.com/xitcoin-org/pos-chain/server" {
   ...
 }
```
...
Add a new field for listeners (uses `ante.PendingTxListener // from "github.com/xitcoin-org/pos-chain/ante"`):
*(app/app.go)*

```diff
 type MyApp struct {
   ...
+  pendingTxListeners []ante.PendingTxListener // from "github.com/xitcoin-org/pos-chain/ante"
 }
```
...
```diff
- ibcTransferPrecompile, err := ics20precompile.NewPrecompile // from ics20precompile "github.com/xitcoin-org/pos-chain/precompiles/ics20"(
-     stakingKeeper,
+ ibcTransferPrecompile, err := ics20precompile.NewPrecompile // from ics20precompile "github.com/xitcoin-org/pos-chain/precompiles/ics20"(
+     bankKeeper,
+     stakingKeeper,
...
-     // from govprecompile "github.com/xitcoin-org/pos-chain/precompiles/gov"(govKeeper, cdc)
+ govPrecompile, err := govprecompile.NewPrecompile
+     // from govprecompile "github.com/xitcoin-org/pos-chain/precompiles/gov"(govKeeper, cdc, options.AddressCodec)
```
...
- [ ] `go.mod` has **no `replace`** lines for `github.com/xitcoin-org/pos-chain`.
