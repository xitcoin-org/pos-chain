...
Status: prepared design; runtime unchanged and no transfer executed.
Related acceptance coordination: https://github.com/xitcoin-org/pos-chain/issues/33.
G03 remains open until an isolated implementation and independent review pass.

## Observed boundary

The repository `server.js` journals a claim only after `runTx` returns a hash.
...
On 13 September 2026 the installed source SHA256 was
...
Review and publish implementation separately, validate only the affected offline
cases plus required CI, and obtain independent acceptance before any deployment.
No live transaction, signing or operational installation is authorized by this
document. The current follow-up does not modify the transaction path.
